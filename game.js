...
function placeholderGround(ctx, width, height, groundHeight, offset) {
  ...
  for (let x = -stripe; x < width + stripe; x += stripe) {
    ctx.fillRect(x - (offset % stripe), height - groundHeight, 12, 6);
  }
  ctx.restore();
}
...
function startGame() {
  ...
  secondsSinceCrash = 0;
  overlay.hidden = true;
  sr.textContent = 'Game started. Score ' + score + '.';
  play('flap');
}
...
function update(seconds) {
  ...
  if (birdBottom >= skyBottom || birdTop <= 0) crash();
  for (const pipe of pipes) {
    const overlapsX = birdRight > pipe.x && birdLeft < pipe.x + CONFIG.pipeWidth;
    const hitsTop = birdTop < pipe.gapTop;
    const hitsBottom = birdBottom > pipe.gapBottom;
...
function drawFrame(time) {
  const skyHeight = CONFIG.canvasHeight - CONFIG.groundHeight;
  drawBackground(ctx, CONFIG.canvasWidth, CONFIG.canvasHeight, reduceMotion ? 0 : time);
  for (const pipe of pipes) {
    drawPipe(ctx, pipe.x, pipe.gapTop, pipe.gapBottom, CONFIG.pipeWidth, skyHeight);
  }
